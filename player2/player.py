...
try:
  open('player2/test.txt')
  path1 = 'player2/training-9k.txt'
  path2 = 'player2/test-1k.txt'
except:
  path1 = 'player1/training-9k.txt'
  path2 = 'player1/test-1k.txt'
...
THE_DATA = PyML.VectorDataSet(path1, labelsColumn = -1)
THE_DATA_TWO = PyML.VectorDataSet(path2, labelsColumn = -1)
...
def get_move(view):
  ...
  if len(current) > 0 and current != None:
    ...
    if  testing_value_two.getPredictedClass()[0] == 1 and testing_value.getPredictedClass()[0] == 1:
    ...
    #neighbors=[0, current place]
      ...
      if poi_counter > k/2:
       ...
       #make it return poi
        ...
        if  testing_value.getPredictedClass()[0] == 1 and testing_value_two.getPredictedClass()[0] == 1:
          eat = True
